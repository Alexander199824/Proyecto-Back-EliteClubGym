...
// configuro la conexión a PostgreSQL y el sistema de migración manual
...
// Función para recrear todas las tablas si RECREATE_TABLES=true
async function recreateTablesIfRequested() {
  try {
    ...
    if (shouldRecreate) {
      ...
      // Paso 1: Importar todos los modelos primero
      ...
      // Paso 3: Recrear todas las tablas
...
// Función para importar todos los modelos dinámicamente - CORREGIDA
async function importAllModels() {
  ...
  if (associationErrors.length > 0) {
    ...
    throw new Error(`Errores en asociaciones: ${associationErrors.map(e => e.modelName).join(', ')}`);
  }
